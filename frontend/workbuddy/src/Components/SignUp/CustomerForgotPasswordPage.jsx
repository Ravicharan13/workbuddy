...
const ForgotPasswordPage = () => {
  ...
  try {
    const response = await axios.post("http://localhost:5000/api/auth/customer/send-resetcode", { email });
  ...
  try {
    const response = await axios.post("http://localhost:5000/api/auth/customer/verify-resetcode", {
      email,
      code: fullCode
    });
  ...
  try {
    const response = await axios.post("http://localhost:5000/api/auth/customer/reset-password", {
      email,
...
<div className="text-center mt-6">
  {step === 1 && (
    <p className="text-sm text-gray-500 dark:text-gray-400">
      ...
      >
        <Link to="/customerauth?mode=login">Back to Login</Link>
      </span>
    </p>
  )}
